...
import { saveActivityResult as saveAttempt } from '@/utils/activityResultsDb';
...
function estimateDbFromMetering(metering: number | undefined, actionName: string) {
  ...
  if (action.includes('pen') || action.includes('pencil')) {
    ...
    action.includes('feet') ||
    action.includes('foot')
  ) {
    minimumDb = 60;
    maximumDb = 85;
  } else if (
    action.includes('talk') ||
    action.includes('speaking') ||
...
function getSoundRisk(db: number): SoundRisk {
  if (db < 30) {
    return {
      label: 'No risk',
      example: 'Whisper, quiet library',
      message: 'This sound level is very quiet and safe.',
    };
  }

  if (db < 60) {
    return {
      label: 'Safe for long periods',
      example: 'Normal conversation, classroom noise',
      message: 'This sound level is usually safe for long periods.',
    };
  }
  ...
  if (db < 85) {
    return {
      ...
      example: 'Busy traffic, vacuum cleaner',
      message: 'This is generally safe, but long exposure can cause fatigue.',
    };
  }

  if (db < 90) {
    return {
      label: 'Hearing damage possible',
      example: 'Lawn mower, loud classroom, heavy traffic',
      message: 'Hearing damage is possible after long exposure.',
    };
  }

  if (db < 100) {
    return {
      label: 'Hearing damage likely',
      example: 'Motorbike, power tools, loud music',
      message: 'Hearing damage may happen after short exposure.',
    };
  }

  if (db < 110) {
    return {
      label: 'Serious hearing damage risk',
      example: 'Nightclub, rock concert, chainsaw',
      message: 'Serious hearing damage can happen within minutes.',
    };
  }

  if (db < 120) {
    return {
      label: 'Immediate damage possible',
      example: 'Siren close by, car horn at 1 m',
      message: 'This level can be painful and immediate damage is possible.',
    };
  }

  if (db < 140) {
    return {
      label: 'Immediate severe damage',
      example: 'Jet engine close',
      message: 'Immediate and severe hearing damage is possible.',
    };
  }

  return {
    label: 'Instant permanent damage risk',
    example: 'Explosion, gunshot',
    message: 'This sound level can cause instant permanent damage.',
  };
}
...
export default function ActivityTwoGame() {
  ...
  useEffect(() => {
    earMuffAnswerRef.current = earMuffAnswer;
  }, [earMuffAnswer]);
  ...
  useEffect(() => {
    async function loadDraft() {
      try {
        ...
        if (draftData?.actionName) {
          setActionName(draftData.actionName);
          actionNameRef.current = draftData.actionName;
        }

        if (draftData?.locationName) {
          setLocationName(draftData.locationName);
          locationNameRef.current = draftData.locationName;
        }
        ...
        if (draftData?.surprises) {
          ...
          surprisesRef.current = draftData.surprises;
        }

        if (draftData?.earMuffAnswer) {
          setEarMuffAnswer(draftData.earMuffAnswer);
          earMuffAnswerRef.current = draftData.earMuffAnswer;
        }

        if (draftData?.actionName || draftData?.prediction) {
  ...
  const startRecording = async () => {
    ...
    if (savedResultId !== null) {
      Alert.alert(
        ...
        'This sound result has already been saved. Press Clear Test to record a new sound.'
      );
      return;
    }
    ...
    if (!hasPermission) {
      ...
      if (!permission.granted) {
        Alert.alert(
          ...
          'Please allow microphone access to measure sound.'
        );
        return;
      }
    }
    ...
    try {
      ...
      recording.setOnRecordingStatusUpdate((status) => {
        ...
        const estimatedDb = estimateDbFromMetering(
          status.metering,
          actionNameRef.current.trim()
        );
  ...
  const stopRecording = async () => {
    ...
    try {
      ...
      try {
        ...
        console.log('Activity 2 saved locally but Firestore failed:', firestoreError);
        Alert.alert(
          'Saved Locally',
          'The sound result was saved locally, but Firestore upload failed.'
        );
      }
      ...
      Alert.alert(
        'Save Failed',
        'The sound result could not be saved. Please try again.'
      );
    } finally {
      setIsSaving(false);
    }
  ...
  const openLeaderboard = () => {
    router.push('/leaderboard?activityKey=activity-two' as never);
  };
  ...
  const renderActivityTab = () => (
    ...
    >
      ...
      >
        ...
        <Text style={[styles.body, { color: colors.subtitle }]}>
          4. Press Stop Recording and Save.
        </Text>
        <Text style={[styles.body, { color: colors.subtitle }]}>
          5. Compare the maximum dB with the hearing risk table.
        </Text>
      </View>
      ...
      <View style={styles.draftRow}>
        ...
        {actionName.trim().length > 0 && (
          <Pressable onPress={clearDraft}>
            <Text style={[styles.clearDraftText, { color: DANGER_COLOR }]}>
              Clear Draft
            </Text>
          </Pressable>
        )}
      ...
      {savedResultId === null ? (
        <Pressable
          ...
          style={({ pressed }) => [
            styles.button,
            { backgroundColor: isRecording ? DANGER_COLOR : colors.tint },
            pressed && styles.buttonPressed,
          ]}
        >
        ...
        <Text style={[styles.savedText, { color: SUCCESS_COLOR }]}>
          Sound result saved to Result History and Firestore.
        </Text>
      )}
        ...
        <View style={[styles.resultsBox, { borderColor: colors.border }]}>
          ...
          {results.map((result) => (
            ...
            >
              ...
              <Text style={[styles.score, { color: SUCCESS_COLOR }]}>
                Maximum: {result.maximumSoundDb.toFixed(1)} dB
              </Text>

              <Text style={[styles.body, { color: colors.subtitle }]}>
                Risk: {result.hearingRisk}
              </Text>
            </View>
          ))}
        </View>
  ...
  const renderWriteUpTab = () => (
    ...
    >
      ...
      <Text style={[styles.body, { color: colors.subtitle }]}>
        Predict which action creates the loudest sound. Then compare your
        prediction with the measured maximum dB.
      </Text>
      ...
      {results.length > 0 && (
        ...
        >
          ...
          {results.map((result) => (
            ...
            >
              <Text style={[styles.tableCell, { color: colors.text, flex: 2 }]}>
                {result.actionName}
              </Text>
              ...
              <Text style={[styles.tableCell, { color: SUCCESS_COLOR, flex: 2 }]}>
                {result.hearingRisk}
              </Text>
            </View>
          ))}
  ...
  const renderDiscussionTab = () => (
    ...
    >
      ...
      >
        ...
        <Text style={[styles.body, { color: colors.subtitle }]}>
          Sound pollution is unwanted or harmful sound. Loud sounds can affect
          hearing, concentration, communication, and comfort.
        </Text>
      </View>
      ...
      >
        ...
        <Text style={[styles.body, { color: colors.subtitle }]}>
          The phone microphone estimates sound level in decibels. The app saves
          the maximum dB, location, prediction, and hearing risk.
        </Text>
      </View>
      ...
      {RISK_TABLE.map((row) => (
        ...
        >
          <Text style={[styles.riskRange, { color: colors.text }]}>
            {row.range}
          </Text>
      ...
      >
        <Text style={[styles.secondaryButtonText, { color: DANGER_COLOR }]}>
          Clear Test
        </Text>
      </Pressable>
