...
function createTeamDiscriminator(): string {
...
export default function TeamSetupScreen() {
  ...
  const updateMemberCount = (value: string) => {
    ...
    if (Number.isNaN(parsedCount) || parsedCount < 1 || parsedCount > MAX_TEAM_MEMBERS) return;
    setMemberNames((current) => {
      if (parsedCount > current.length) {
        return [...current, ...Array(parsedCount - current.length).fill('')];
      }
      return current.slice(0, parsedCount);
    });
  };
  ...
  const saveTeam = async () => {
    ...
    if (!cleanGradeLevel) {
      Alert.alert('Missing Grade Level', 'Please enter your grade or year level.');
      return;
    }
    if (cleanMemberNames.length === 0) {
      Alert.alert('Missing Members', 'Please enter at least one team member name.');
      return;
    }
    ...
    try {
      ...
      await saveLocalTeamProfile(teamProfile);
  ...
  teamDiscriminator,
        ...
        // Firestore save failed — local profile still saved
      }
      ...
      console.log('Failed to save team:', error);
      Alert.alert('Save Failed', 'Could not save your team. Please try again.');
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const searchForTeam = async () => {
    ...
    try {
      ...
      if (!team) {
        Alert.alert('Team Not Found', 'No team with that code was found. Check the code and try again.');
        return;
      }
      ...
      Alert.alert('Search Failed', 'Could not search for the team. Check your connection and try again.');
    } finally {
  ...
  const joinFoundTeam = async () => {
    ...
    try {
      ...
      const teamProfile = {
        teamId: foundTeam.teamDiscriminator,
        teamName: foundTeam.teamName,
        memberCount: foundTeam.memberNames.length,
      ...
      Alert.alert(
        'Team Joined',
        `You have joined ${foundTeam.teamName}!`,
        [
          {
            text: 'Continue',
            onPress: async () => {
  ...
  const renderCreateTab = () => (
    <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
      ...
      <TextInput
        ...
        placeholder="e.g. Galaxy Squad"
        placeholderTextColor={colors.subtitle}
        style={[styles.input, { color: colors.text, borderColor: colors.border, backgroundColor: colors.background }]}
      />

      <Text style={[styles.label, { color: colors.text }]}>Grade / Year Level</Text>
      <TextInput
        value={gradeLevel}
      ...
      <Text style={[styles.label, { color: colors.text }]}>Number of Team Members</Text>
      <TextInput
        value={memberCount}
      ...
      {countValue > 0 && countValue <= MAX_TEAM_MEMBERS && (
        <>
          <Text style={[styles.label, { color: colors.text }]}>Member First Names</Text>
          {memberNames.slice(0, countValue).map((name, index) => (
            <TextInput
              key={index}
              value={name}
      ...
      <View style={[styles.discriminatorBox, { borderColor: colors.tint, backgroundColor: `${colors.tint}10` }]}>
        ...
        <Text style={[styles.discriminatorValue, { color: colors.tint }]}>{teamDiscriminator}</Text>
        <Text style={[styles.helperText, { color: colors.subtitle }]}>
          Team members can use this code to join your team from the Join Team tab.
        </Text>
      </View>
  ...
  const renderJoinTab = () => (
    <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
      ...
      <Text style={[styles.body, { color: colors.subtitle }]}>
        Enter the team code given to you by your team leader, e.g. STEMM-1234.
      </Text>

      <Text style={[styles.label, { color: colors.text }]}>Team Code</Text>
      <TextInput
        value={joinCode}
        onChangeText={setJoinCode}
        placeholder="e.g. STEMM-1234"
        placeholderTextColor={colors.subtitle}
      ...
      {foundTeam && (
        <View style={[styles.foundTeamBox, { borderColor: colors.success, backgroundColor: `${colors.success}10` }]}>
          ...
          <Pressable
            ...
            style={({ pressed }) => [
              styles.button,
              { backgroundColor: isSaving ? colors.subtitle : colors.tint, marginTop: 14 },
              pressed && styles.buttonPressed,
            ]}
          >
            <Text style={[styles.buttonText, { color: colors.buttonText }]}>
